...
usage() {
  cat <<'USAGE'
Usage:
  gear_sonic_deploy/scripts/build_a3_deploy_pkg.sh --arch x86_64 [--runtime-cfg PATH]
  gear_sonic_deploy/scripts/build_a3_deploy_pkg.sh --arch rockchip [--runtime-cfg PATH]

Options:
  --arch ARCH          x86_64 or rockchip.
  --runtime-cfg PATH   Source A3 runtime YAML. Defaults to the repo A3 config.
  --smpl-zmq-host HOST Override packaged smpl_zmq.host. The source setting is
                       otherwise preserved; supply this explicitly when an
                       optional SMPL sender is on another host.
  --onnxruntime-aarch64-tarball PATH
                      Local aarch64 ONNX Runtime archive for a Rockchip build.
                      Required for --arch rockchip; alternatively set
                      A3_ONNXRUNTIME_AARCH64_TARBALL.
...
Rockchip sysroot:
  Rockchip builds use the Hugging Face-hosted
  thirdparty/rockchip_sysroot/rockchip-1.0-aarch64-sysroot.tar.gz.
  From the repository root: python download_from_hf.py --component sysroot
  To retarget the sysroot, regenerate it from a local matching NVIDIA Thor
  base image with gear_sonic_deploy/scripts/export_rockchip_sysroot.sh
  --image <local-image>. Set A3_ROCKCHIP_SYSROOT_TARBALL_REL to use a
  different repo-relative path.

Rockchip ONNX Runtime:
  Rockchip builds also require an externally supplied aarch64 ONNX Runtime
  archive. It must contain exactly one
  */lib/cmake/onnxruntime/onnxruntimeConfig.cmake install config. This
  repository does not distribute that host-specific binary.
  The x86_64 cross-builder base defaults to debian:bookworm, matching the
  target build assumptions. Override with A3_ROCKCHIP_BUILDER_BASE.
USAGE
}
...
ARCH=""
RUNTIME_CFG="${GEAR_ROOT}/src/g1/g1_deploy_onnx_ref/config/a3_runtime_config.yaml"
SMPL_ZMQ_HOST_OVERRIDE=""
INSIDE_DOCKER=0
BUILD_ONLY=0
JOBS="$(nproc)"
ONNXRUNTIME_AARCH64_TARBALL="${A3_ONNXRUNTIME_AARCH64_TARBALL:-}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --smpl-zmq-host)
      ...
      shift 2
      ;;
    --onnxruntime-aarch64-tarball)
      ONNXRUNTIME_AARCH64_TARBALL="${2:-}"
      shift 2
      ;;
    --jobs)
      JOBS="${2:-}"
...
case "${ARCH}" in
  ...
  rockchip)
    ...
    ;;
  *)
    echo "--arch must be one of: x86_64, rockchip" >&2
    exit 64
    ;;
esac
...
clear_stale_cross_cmake_cache_if_needed() {
  ...
  if [[ "${ARCH}" != "rockchip" || ! -f "${BUILD_DIR}/CMakeCache.txt" ]]; then
    return 0
  fi
...
From the repository root, fetch the Hugging Face asset (SHA-256 checked):
  python download_from_hf.py --component sysroot

If the target ABI requires a refreshed sysroot, generate it from a matching
local NVIDIA Thor target image:
  A3_THOR_BASE_IMAGE=<local-nvidia-thor-image> \\
    gear_sonic_deploy/scripts/export_rockchip_sysroot.sh \\
      --image "\$A3_THOR_BASE_IMAGE"

Or set A3_ROCKCHIP_SYSROOT_TARBALL_REL to another path relative to gear_sonic_deploy.
EOF
  exit 66
}

require_onnxruntime_aarch64_bundle() {
  if [[ -n "${ONNXRUNTIME_AARCH64_TARBALL}" && -f "${ONNXRUNTIME_AARCH64_TARBALL}" ]]; then
    return 0
  fi

  cat >&2 <<EOF
missing aarch64 ONNX Runtime archive for Rockchip build

Obtain or build a target-compatible ONNX Runtime 1.19.2 package outside this
repository, then provide it explicitly:
  gear_sonic_deploy/scripts/build_a3_deploy_pkg.sh --arch rockchip \\
    --onnxruntime-aarch64-tarball /absolute/path/to/onnxruntime-aarch64-1.19.2.tar.gz

or set A3_ONNXRUNTIME_AARCH64_TARBALL to that path. The archive must contain
exactly one */lib/cmake/onnxruntime/onnxruntimeConfig.cmake install config.
EOF
  exit 66
}
...
build_rockchip_in_docker() {
  ...
  local onnxruntime_tarball=""
  local status=0
  require_rockchip_sysroot_bundle
  require_onnxruntime_aarch64_bundle
  onnxruntime_tarball="$(absolute_existing_file "${ONNXRUNTIME_AARCH64_TARBALL}")"
  ...
  docker_run_args+=(
    -e "A3_ONNXRUNTIME_AARCH64_TARBALL=/tmp/a3_onnxruntime_aarch64.tar.gz"
    -v "${onnxruntime_tarball}:/tmp/a3_onnxruntime_aarch64.tar.gz:ro"
  )
  ...
  if [[ "${status}" -eq 0 ]]; then
    docker run --rm --platform linux/amd64 \
      ...
      -w /work \
      "${image}" \
      bash -lc "source /opt/ros/jazzy/setup.bash && gear_sonic_deploy/scripts/build_a3_deploy_pkg.sh --arch rockchip --inside-docker --build-only --jobs '${JOBS}'" || status=$?
  fi
...
configure_and_build() {
  ...
  if [[ "${ARCH}" == "rockchip" ]]; then
    ...
    cmake_args+=(
      ...
      "-DGS_ONNXRUNTIME_AARCH64_TARBALL=${ONNXRUNTIME_AARCH64_TARBALL}"
...
copy_onnxruntime_lib_dir() {
  local lib_dir="$1"
  [[ -d "${lib_dir}" ]] || return 1
  if ! find "${lib_dir}" -maxdepth 1 \( -type f -o -type l \) -name 'libonnxruntime*.so*' -print -quit 2>/dev/null | grep -q .; then
    return 1
  fi
  copy_found_libs "${lib_dir}" "libonnxruntime*.so*"
}

stage_x86_64_onnxruntime_libs() {
  [[ "${ARCH}" == "x86_64" ]] || return 0

  local resolved_libs=()
  local exe lib cache_lib real_lib lib_dir copied=0

  for exe in "${PKG_DIR}/a3_deploy_onnx_ref" "${PKG_DIR}/a3_policy_runtime_probe"; do
    [[ -x "${exe}" ]] || continue
    while IFS= read -r lib; do
      [[ "${lib}" == /* && -e "${lib}" ]] || continue
      real_lib="$(readlink -f "${lib}")"
      [[ -n "${real_lib}" && -f "${real_lib}" ]] || continue
      resolved_libs+=("${real_lib}")
    done < <(
      ldd "${exe}" 2>/dev/null | awk '
        /libonnxruntime.*=>/ && $3 != "not" { print $3 }
        /^[[:space:]]*\/.*libonnxruntime.*\.so/ { print $1 }
      '
    )
  done

  if [[ -f "${BUILD_DIR}/CMakeCache.txt" ]]; then
    cache_lib="$(sed -n 's/^onnxruntime_LIBRARY:FILEPATH=//p' "${BUILD_DIR}/CMakeCache.txt" | head -n 1)"
    if [[ -n "${cache_lib}" && -e "${cache_lib}" ]]; then
      real_lib="$(readlink -f "${cache_lib}")"
      [[ -n "${real_lib}" && -f "${real_lib}" ]] && resolved_libs+=("${real_lib}")
    fi
  fi

  for lib in "${resolved_libs[@]}"; do
    lib_dir="$(dirname "${lib}")"
    if copy_onnxruntime_lib_dir "${lib_dir}"; then
      copied=1
    fi
  done
  ...
  if [[ "${copied}" -ne 1 ]]; then
    cat >&2 <<EOF
unable to stage x86_64 ONNX Runtime shared libraries into ${PKG_DIR}

The a3_deploy_onnx_ref binary links libonnxruntime dynamically. Install or expose
the same ONNX Runtime used for the build, then rerun this packaging script.
EOF
    exit 1
  fi
}
...
def auto_rknn_candidates(onnx_src):
    ...
    return unique_existing([
        ...
        repo_root / "gear_sonic_deploy" / "assets" / "a3_runtime" / "rknn_models" / rknn_name,
...
def resolve_rknn_model(raw, label, onnx_src, required=False):
    ...
    if required:
        ...
        raise SystemExit(
            ...
            "gear_sonic_deploy/assets/a3_runtime/rknn_models/ or next to the ONNX. "
            "Run gear_sonic_deploy/scripts/convert_a3_onnx_to_rknn.py first."
        )
    return None
...
rockchip_auto_rknn_backend = arch == "rockchip"
...
if arch == "rockchip":
    set_nested(cfg, ["policy_driver", "rt", "sched_fifo_priority"], 80)
    # Pin the Rockchip policy RT thread to CPU 6 with FIFO priority 80.
    # Launch with taskset -c 4-5; this explicit per-thread affinity overrides
    # the initial affinity inherited by the policy thread.
    set_nested(cfg, ["policy_driver", "rt", "cpu_affinity"], 6)
...
PY
  ...
  local fastrtps_src="${GEAR_ROOT}/src/g1/g1_deploy_onnx_ref/config/fastrtps_profile.xml"
...
stage_body_drive_debug_files() {
  local debug_src="${GEAR_ROOT}/src/g1/g1_deploy_onnx_ref"

  mkdir -p \
    "${PKG_DIR}/config" \
    "${PKG_DIR}/tools" \
    "${PKG_DIR}/assets/robot_description/urdf"
  ...
  install -m 0755 \
    ...
    "${debug_src}/scripts/tools/a3_eval_cli.py" \
    "${debug_src}/scripts/tools/serve_foxglove_assets.py" \
    "${PKG_DIR}/tools/"

  local a3_urdf_src="${REPO_ROOT}/gear_sonic/data/assets/robot_description/urdf/a3"
  if [[ ! -f "${a3_urdf_src}/model.urdf" ]]; then
    echo "missing A3 URDF assets: ${a3_urdf_src}" >&2
    exit 1
  fi
  rm -rf "${PKG_DIR}/assets/robot_description/urdf/a3"
  cp -a "${a3_urdf_src}" "${PKG_DIR}/assets/robot_description/urdf/"
}
...
A3_ROBOT_ENV="${A3_ROBOT_ENV:-}"
A3_SOURCE_ROBOT_ENV_ENABLED="${A3_SOURCE_ROBOT_ENV:-${A3_SOURCE_ROBOT_ENV_DEFAULT}}"
if [[ "${A3_SOURCE_ROBOT_ENV_ENABLED}" != "0" ]]; then
  if [[ -z "${A3_ROBOT_ENV}" ]]; then
    echo "robot environment requested but A3_ROBOT_ENV is unset; set it to the vendor env script or use A3_SOURCE_ROBOT_ENV=0 to skip" >&2
    exit 64
  fi
...
exec "${SCRIPT_DIR}/run_a3.sh" \
  --probe \
  --probe-source "${A3_PROBE_SOURCE:-g1}" \
...
stage_extra_libs() {
  ...
  copy_found_libs "${BUILD_DIR}" "libaimrt_net_plugin.so*"
...
stage_joint_msgs_ros2_cli_overlay() {
  # deploy binary 只需 package root 中的 C/C++ type-support；Foxglove bridge
  # 还需要通过 ament index 找到自定义消息 schema。Python generator 在部分
  # 构建环境会被主动关闭，因此基础 overlay 不应依赖 Python 产物存在。
  local joint_build="${BUILD_DIR}/src/g1/g1_deploy_onnx_ref/joint_msgs_build"
  ...
  # rclcpp GenericSubscription/Foxglove 会从 prefix/lib 动态加载
  # joint_msgs introspection type-support。
  mkdir -p "${PKG_DIR}/lib"
  find "${PKG_DIR}" -maxdepth 1 -type f -name 'libjoint_msgs__*.so' \
    -exec cp -f {} "${PKG_DIR}/lib/" \;
...
verify_package() {
  ...
  for exe in "${PKG_DIR}/a3_deploy_onnx_ref" "${PKG_DIR}/a3_policy_runtime_probe"; do
    while IFS= read -r needed; do
      if [[ "${needed}" == libonnxruntime*.so* && ! -e "${PKG_DIR}/${needed}" ]]; then
        echo "missing packaged ONNX Runtime library required by $(basename "${exe}"): ${needed}" >&2
        exit 1
      fi
    done < <(readelf -d "${exe}" 2>/dev/null | sed -n 's/.*Shared library: \[\([^]]*\)\].*/\1/p')
  done
  local debug_required=(
    ...
    "tools/a3_eval_cli.py"
    "tools/serve_foxglove_assets.py"
    "assets/robot_description/urdf/a3/model.urdf"
  )
  local rel
...
if arch == "rockchip":
    rt_cfg = cfg.get("policy_driver", {}).get("rt", {})
    if int(rt_cfg.get("sched_fifo_priority", -1)) != 80:
        raise SystemExit("Rockchip package must set policy_driver.rt.sched_fifo_priority=80")
    if int(rt_cfg.get("cpu_affinity", -1)) != 6:
        raise SystemExit("Rockchip package must set policy_driver.rt.cpu_affinity=6")
PY
}

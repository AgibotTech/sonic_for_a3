...
//
// Historical note: pre 2026-04 these arrays were stored in G1's flat layout
...
// Released 024 actuator gains. Only the four ankle entries differ from the
// base A3 gains above.
constexpr auto a3_024_kps = [] {
  auto gains = a3_kps;
  for (const int i : {21, 22, 27, 28}) gains[i] = 60.0;
  return gains;
}();

constexpr auto a3_024_kds = [] {
  auto gains = a3_kds;
  for (const int i : {21, 22, 27, 28}) gains[i] = 1.2;
  return gains;
}();
...
// bring-up state uses the production stand gains recorded for the A3 release
// target during bring-up verification.
//
...
// Released 024 position-action scale. The waist/knee effort limits and ankle
// stiffness define the scale; ankle damping does not affect it.
constexpr auto a3_024_action_scale = [] {
  auto scale = a3_action_scale;
  scale[0] = 0.25 * 200.0 / 85.0;   // waist_yaw
  scale[1] = 0.25 * 42.0 / 50.0;    // waist_roll
  scale[2] = 0.25 * 105.0 / 50.0;   // waist_pitch
  scale[20] = 0.25 * 300.0 / 250.0; // left_knee
  scale[21] = 0.25 * 118.2 / 60.0;  // left_ankle_pitch
  scale[22] = 0.25 * 54.75 / 60.0;  // left_ankle_roll
  scale[26] = 0.25 * 300.0 / 250.0; // right_knee
  scale[27] = 0.25 * 118.2 / 60.0;  // right_ankle_pitch
  scale[28] = 0.25 * 54.75 / 60.0;  // right_ankle_roll
  return scale;
}();

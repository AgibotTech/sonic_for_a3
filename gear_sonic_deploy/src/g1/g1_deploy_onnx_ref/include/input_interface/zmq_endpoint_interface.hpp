...
#include "packed_motion_decoder.hpp"
...
private:
    ...
    DecodeResult DecodeIntoMotionSequence(int current_playback_frame, 
        ...
        // Keep the legacy G1 optional-field handling below, but validate the
        // core v1/v2/v3 packed motion payload through the shared decoder used
        // by A3. This keeps both consumers on the same transport contract.
        input_interface::PackedMotionDecodeResult shared_decoded;
        if (!input_interface::DecodePackedMotionMessage(
                buffered_header_, buffered_buffers_, shared_decoded)) {
            std::cerr << "[ZMQEndpointInterface] Packed motion decode failed: "
                      << shared_decoded.error << std::endl;
            return result;
        }

...
//       + a3_default_angles[i]
// This matches G1's decoder (g1_deploy_onnx_ref.cpp:3089-3091) modulo the
// different MuJoCo axis convention between G1 (Unitree flat) and A3
// (MuJoCo real).
//
...
// Shared bound for decoded policy actions and the cached `last_actions`
// observation. The cache must contain the same clipped raw action used for
// the physical command.
inline constexpr float kA3RawActionClip = 20.0f;
...
// Explicit-scale overload used by experiment-specific deployment profiles.
void DecodeAction(const std::array<float, 29>& raw_action_isaaclab,
                  const std::array<double, 29>& action_scale_mujoco,
                  std::array<double, 29>& q_des_mujoco) noexcept;

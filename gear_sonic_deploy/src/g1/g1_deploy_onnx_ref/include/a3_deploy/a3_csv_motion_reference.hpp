...
// Evaluation recorder 使用的当前 reference 快照。关节统一输出 MuJoCo
// 29-DoF policy-view 顺序；pelvis pose 使用米和 WXYZ quaternion。
struct A3EvalReferenceSample {
  std::array<float, 29> q_mujoco{};
  std::array<float, 29> dq_mujoco{};
  std::array<float, 3> pelvis_position_m{};
  std::array<float, 4> pelvis_quat_wxyz{1.0f, 0.0f, 0.0f, 0.0f};
};
...
class A3CsvMotionReference {
 public:
  ...
  // 只读、无内存分配的单帧采样接口；不会推进 reference 播放状态。
  bool SampleEvalFrame(std::size_t tick_idx,
                       A3EvalReferenceSample& out) const noexcept;
  // 记录侧使用的重载：对 pelvis pose 应用与 tokenizer 相同的固定 yaw
  // 对齐，使 MCAP 中的 orientation 与实际送入 policy 的 reference 一致。
  bool SampleEvalFrame(std::size_t tick_idx,
                       double reference_yaw_offset_rad,
                       A3EvalReferenceSample& out) const noexcept;

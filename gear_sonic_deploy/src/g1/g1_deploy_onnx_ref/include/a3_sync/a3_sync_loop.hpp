...
class A3SyncLoop {
 public:
  ...
  // Latest-cache mode only: build a state at the caller's control cadence.
  // Subscriber callbacks update independent lock-free raw caches; a pull is
  // intentionally a best-effort cross-channel snapshot, not synchronization.
  bool TryReadLatestState(robot_io::RobotState& state);

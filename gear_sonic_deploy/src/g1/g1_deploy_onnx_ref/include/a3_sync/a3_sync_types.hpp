...
enum class SyncMode : std::uint8_t {
  ...
  MinSkewPair  = 1,
  LatestFrame  = 2,
  // Do not synthesize on subscriber callbacks.  Each callback only replaces
  // its own latest sample; the policy clock pulls and assembles a snapshot.
  LatestCache  = 3,
};
...
struct SyncConfig {
  ...
  // A3 production defaults to independent per-topic latest caches sampled by
  // the fixed policy clock.  Coherent min-skew pairing remains opt-in.
  SyncMode sync_mode{SyncMode::LatestCache};

...
struct RobotState {
  ...
  // Primary IMU (A3 /pelvis_imu/data).
...
class RobotIOBackend {
 public:
  ...
  // Initialise the A3 AimRT transport. `config` is backend-specific.
  ...
  // Optional pull-state path. Backends that return true from
  // UsesLatestStatePull() keep raw topic caches themselves and let the
  // policy's fixed-rate control tick read a best-effort latest snapshot. The
  // individual input channels are intentionally not globally synchronized.
  // This avoids constructing/copying a full RobotState for every raw input.
  virtual bool UsesLatestStatePull() const noexcept { return false; }
  virtual bool TryReadLatestState(RobotState& state) { return false; }

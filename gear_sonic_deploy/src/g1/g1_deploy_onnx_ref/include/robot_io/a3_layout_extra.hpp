...
// does not modify any G1 data structures.
...
// RobotCommand/RobotState. Do NOT confuse with the G1 29-DOF layout
...
// with body joints in G1-compatible order [0..28]. We realigned to MuJoCo's
...
//
// ⚠️ Not the same order as G1's 29-DOF flat layout — G1 uses
//   legs(12) + waist(3) + arms(14) which coincides with Unitree SDK
//   motor_state order and G1 MuJoCo. A3 has no such coincidence.
...
//   kG1ToA3NeckIdx[i] == j  ⇒  flat layout neck sub-index `i` (i.e. the
//                              31-DOF slot kA3NeckStart + i) lands at
//                              position `j` inside the A3 neck topic.
extern const std::array<int, kA3NeckCount> kG1ToA3NeckIdx;
extern const std::array<int, kA3NeckCount> kA3NeckToG1Idx;

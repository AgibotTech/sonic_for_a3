...
class A3AimrtBackend : public RobotIOBackend {
 public:
  ...
  // The default timing path is latest_cache: each of the six subscribers keeps
  // its newest sample and the fixed 50Hz policy clock pulls one snapshot.  A
  // stale channel still makes the pull invalid; min_skew_pair is opt-in when a
  // coherently paired state stream is required.
  ...
  //   sync_mode=latest_cache|latest_frame|header_interp|min_skew_pair
  //   sync_hz=<double>
...
#if defined(HAS_A3_ROS_MSGS) && defined(HAS_A3_HAND_PROTO)
  using HandCommandCaptureFn =
      std::function<void(std::int64_t stamp_ns, std::uint32_t seq,
                         const aimdk::protocol::HandCommandChannel& msg)>;
  using HandStateCaptureFn =
      std::function<void(std::int64_t stamp_ns,
                         const sensor_msgs::msg::JointState& msg)>;

  void SetHandCommandCaptureFn_ForTest(HandCommandCaptureFn fn);
  void SetHandStateCaptureFn_ForTest(HandStateCaptureFn fn);
  bool InjectMotionHandCommand_ForTest(
      const sensor_msgs::msg::JointState& msg);
  bool InjectBodyDriveHandState_ForTest(
 ...
 private:
...
#if defined(HAS_A3_ROS_MSGS) && defined(HAS_A3_HAND_PROTO)
  bool HandleMotionHandCommand_(const sensor_msgs::msg::JointState& msg);
  bool HandleBodyDriveHandState_(
      const aimdk::protocol::HandStateChannel& msg);
#endif
  ...
  bool               hand_bridge_enabled_{true};
  ...
  std::shared_ptr<a3_deploy::A3MotionRpcController> motion_rpc_controller_;

#if defined(HAS_A3_ROS_MSGS) && defined(HAS_A3_HAND_PROTO)
  using HandCommandPublishFn =
      std::function<void(const aimdk::protocol::HandCommandChannel&)>;
  using HandStatePublishFn =
      std::function<void(const sensor_msgs::msg::JointState&)>;

  HandCommandPublishFn hand_command_publish_fn_{};
  HandStatePublishFn hand_state_publish_fn_{};
  HandCommandCaptureFn hand_command_capture_fn_{};
  HandStateCaptureFn hand_state_capture_fn_{};
#endif
  ...
  std::shared_ptr<void>                    motion_rpc_service_{};
#endif

  std::atomic<std::uint32_t> seq_counter_{0};
#if defined(HAS_A3_ROS_MSGS) && defined(HAS_A3_HAND_PROTO)
  std::atomic<std::uint32_t> hand_seq_counter_{0};
#endif

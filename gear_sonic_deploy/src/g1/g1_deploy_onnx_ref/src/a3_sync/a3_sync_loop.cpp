...
void A3SyncLoop::Start() {
  ...
  if (opt_.sync.sync_mode == SyncMode::LatestFrame ||
      opt_.sync.sync_mode == SyncMode::LatestCache) {
    return;
  }
...
void A3SyncLoop::OnWaistState(const WaistSample& s) {
  ...
  if (opt_.sync.sync_mode == SyncMode::LatestCache) {
...
bool A3SyncLoop::TryReadLatestState(robot_io::RobotState& state) {
  if (opt_.sync.sync_mode != SyncMode::LatestCache ||
      !running_.load(std::memory_order_relaxed)) {
    return false;
  }
  return TryBuildLatestFrameLocked(state);
}
...
bool A3SyncLoop::TryBuildLatestFrameLocked(robot_io::RobotState& state) {
  ...
  // latest_cache deliberately does not gate control on cross-topic skew:
  // each channel is independently refreshed at its native rate and the
  // policy accepts the best-effort latest snapshot, matching motion_control.
  // The skew remains in RobotState/statistics for diagnosis.
  const bool aligned = opt_.sync.sync_mode == SyncMode::LatestCache ||
                       skew_ns <= opt_.sync.max_skew_ns;

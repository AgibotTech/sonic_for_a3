...
void A3ObsBuilder::BuildProprioHistory(
  ...
  // gear_sonic/envs/manager_env/mdp/observations.py:70+, NOT by the
...
void A3ObsBuilder::RememberAction(
  ...
  // DecodeAction applies this same bound before producing q_des. Cache the
  // clipped value so the next observation matches the applied command.
  std::array<float, 29> applied_action_isaaclab{};
  for (std::size_t i = 0; i < applied_action_isaaclab.size(); ++i) {
    applied_action_isaaclab[i] = std::clamp(
        raw_action_isaaclab[i], -kA3RawActionClip, kA3RawActionClip);
  }
  IsaacToMujocoD(applied_action_isaaclab, last_action_mujoco_);
}

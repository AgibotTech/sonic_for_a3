...
bool A3CsvMotionReference::SampleEvalFrame(
    std::size_t tick_idx, A3EvalReferenceSample& out) const noexcept {
  return SampleEvalFrame(tick_idx, 0.0, out);
}

bool A3CsvMotionReference::SampleEvalFrame(
    std::size_t tick_idx, double reference_yaw_offset_rad,
    A3EvalReferenceSample& out) const noexcept {
  std::size_t resolved = 0;
  if (!ResolveTick(tick_idx, &resolved)) return false;
  out.q_mujoco = frames_[resolved].dof_mujoco;
  for (std::size_t mj = 0; mj < kPolicyDof; ++mj) {
    const auto isaac = static_cast<std::size_t>(a3_isaaclab_to_mujoco[mj]);
    out.dq_mujoco[mj] = dof_vel_isaaclab_[resolved][isaac];
  }
  const auto& source_position = frames_[resolved].root_pos;
  const double c = std::cos(reference_yaw_offset_rad);
  const double s = std::sin(reference_yaw_offset_rad);
  out.pelvis_position_m = {
      static_cast<float>(c * source_position[0] - s * source_position[1]),
      static_cast<float>(s * source_position[0] + c * source_position[1]),
      static_cast<float>(source_position[2])};
  const auto aligned_quat = NormalizeQuat(
      QuatMul(QuatFromYawRad(reference_yaw_offset_rad),
              frames_[resolved].root_quat_wxyz));
  for (std::size_t i = 0; i < aligned_quat.size(); ++i) {
    out.pelvis_quat_wxyz[i] = static_cast<float>(aligned_quat[i]);
  }
  return true;
}

...
bool IsFiniteArray(const std::array<double, N>& values) noexcept {
  for (double v : values) {
...
void A3TeleopReferenceBuffer::PushFrame(const A3TeleopFrame& frame) {
  if (frame.stamp_ns <= 0 || !IsFiniteArray(frame.q_mujoco) ||
      !IsFiniteArray(frame.dq_mujoco) ||
      (frame.has_head_command &&
       (!IsFiniteArray(frame.head_q_rad) ||
        !IsFiniteArray(frame.head_dq_rad_s))) ||
  ...
  if (!normalized.has_head_command) {
    normalized.head_q_rad.fill(0.0);
    normalized.head_dq_rad_s.fill(0.0);
  }
...
bool A3TeleopReferenceBuffer::SampleDelayedFrame(
    std::int64_t now_ns,
    A3TeleopFrame& out) const noexcept {
  std::lock_guard<std::mutex> lk(mu_);
  if (frames_.empty()) return false;
  return SampleAtLocked(now_ns - options_.delay_ns, out);
}
...
bool A3TeleopReferenceBuffer::SampleAtLocked(
  ...
  out.has_head_command = lo->has_head_command && hi->has_head_command;
  if (out.has_head_command) {
    for (std::size_t i = 0; i < out.head_q_rad.size(); ++i) {
      out.head_q_rad[i] =
          (1.0 - alpha) * lo->head_q_rad[i] + alpha * hi->head_q_rad[i];
      out.head_dq_rad_s[i] =
          (1.0 - alpha) * lo->head_dq_rad_s[i] + alpha * hi->head_dq_rad_s[i];
    }
  } else {
    out.head_q_rad.fill(0.0);
    out.head_dq_rad_s.fill(0.0);
  }
...
bool ConvertTaWholeBodyCommand(
  ...
  const auto& head = data.head_command().angles_rad();
  ...
  if (head.size() != 0 && head.size() != 2) {
    return fail("head_command.angles_rad must have size 2 when present");
  }
  ...
  if (head.size() == 2) {
    frame.has_head_command = true;
    frame.head_q_rad[0] = head.Get(0);
    frame.head_q_rad[1] = head.Get(1);
  }
  ...
  if (vel.size() == 30) {
    ...
    for (int i = 0; i < 3; ++i) frame.dq_mujoco[i] = vel.Get(12 + i);
    frame.head_dq_rad_s[0] = vel.Get(15);
    frame.head_dq_rad_s[1] = vel.Get(16);

...
std::int64_t NowMonotonicNs() noexcept {
  return std::chrono::duration_cast<std::chrono::nanoseconds>(
...
void A3PolicyDriver::OnBackendState_(const robot_io::RobotState& state) noexcept {
  ...
  // This follows the transport-independent backend command pattern.
  try {
...
void A3PolicyDriver::RunOnce() noexcept {
  ...
  if (backend_.UsesLatestStatePull()) {
    if (backend_.TryReadLatestState(pulled_state_)) {
      pulled_state_valid_ = true;
    }
    // Once a valid snapshot has been observed, retain it until the watchdog
    // sees its age exceed the configured safety threshold and sends halt.
    if (!pulled_state_valid_) return;
    RunOnceWithState_(pulled_state_);
    return;
  }
...
void A3PolicyDriver::RunOnceWithState_(
    const robot_io::RobotState& s) noexcept {
  A3PolicyDriverTrace trace;
  trace.policy_tick = policy_tick_count_.load(std::memory_order_relaxed);
  trace.control_start_monotonic_ns = NowMonotonicNs();
  ...
  if (verdict == WatchdogVerdict::Ok && complete) {
    ...
    if (command_) {
      ...
      if (!command_(tick_idx, s, cmd_out_)) {
        trace.control_end_monotonic_ns = NowMonotonicNs();
        EmitTrace_(s, nullptr, trace);
        return;
      }
    } else {
      q_out_buf_.fill(0.0);
    ...
    trace.command_generated = true;
    trace.send_attempted = true;
    trace.send_start_monotonic_ns = NowMonotonicNs();
    trace.send_ok = backend_.SendCommand(cmd_out_);
    trace.send_end_monotonic_ns = NowMonotonicNs();
    has_sent_command_.store(true, std::memory_order_release);
    trace.control_end_monotonic_ns = NowMonotonicNs();
    EmitTrace_(s, &cmd_out_, trace);
    policy_tick_count_.fetch_add(1, std::memory_order_relaxed);
  } else {
    trace.watchdog_safe_halt = true;
...
void A3PolicyDriver::EmitTrace_(
    const robot_io::RobotState& state,
    const robot_io::RobotCommand* command,
    const A3PolicyDriverTrace& trace) noexcept {
  if (!opt_.trace_fn) return;
  try {
    opt_.trace_fn(state, command, trace);
  } catch (...) {
    // 诊断 hook 的异常绝不能改变控制线程或机器人命令行为。
  }
}

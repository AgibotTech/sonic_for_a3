...
// fixed deployment gains by default; teleop overlays TA head after expansion.
...
void ApplyA3HeadCommand(const std::array<double, 2>& head_q_rad,
                        robot_io::RobotCommand& out) {
  if (out.q_des.size() != robot_io::kA3Dof ||
      out.dq_des.size() != robot_io::kA3Dof ||
      out.tau_ff.size() != robot_io::kA3Dof ||
      out.kp.size() != robot_io::kA3Dof ||
      out.kd.size() != robot_io::kA3Dof) {
    return;
  }

  for (int i = 0; i < robot_io::kA3NeckCount; ++i) {
    const int sdk = robot_io::kA3NeckStart + i;
    out.q_des[sdk] = head_q_rad[static_cast<std::size_t>(i)];
    out.dq_des[sdk] = 0.0;
    out.tau_ff[sdk] = 0.0;
    out.kp[sdk] = kA3HeadKp;
    out.kd[sdk] = kA3HeadKd;

...
//
// a3_deploy_onnx_ref — A3 SONIC policy-loop binary.
//
...
bool ValidateKnownFlags(int argc, char** argv, std::string* error) {
  const std::array<std::string, 4> bool_flags = {
      "--dry-run", "--probe", "--probe-manual", "--auto-start"};
...
void PrintUsage(const char* progname) {
  std::cerr << "Usage: " << progname << " --runtime-cfg=/path/to/config.yaml"
               " [--dry-run] [--probe] [--probe-manual] [--auto-start]"
               " [--probe-source g1|smpl|both]"
               " [--aimrt-cfg PATH] [--frame-log-interval N]"
               "\n"
            << "\nSee src/g1/g1_deploy_onnx_ref/config/a3_runtime_config.yaml\n"
            ...
            << "\n--dry-run Start transport/backend only; do not load policy or publish commands.\n"
               ...
               "--probe-manual  Keep the keyboard state machine active during --probe.\n"
               "                Command publishers remain disabled; requires --probe.\n"
               "--probe-source g1|smpl|both  Select probe inference source; default g1.\n"
...
std::string BuildBackendConfigString(const YAML::Node& backend,
  ...
  const bool hand_bridge_enabled =
      OptionalKey<bool>(backend["hand_bridge_enabled"], true);
  add(std::string{"hand_bridge_enabled="} +
      (hand_bridge_enabled ? "true" : "false"));
  return ss.str();
}
...
struct InferenceTimingStats {
  std::atomic<std::int64_t> last_infer_start_ns{0};
  AtomicLatencyBucket policy_period{};
  ...
  AtomicLatencyBucket infer_g1{};
  AtomicLatencyBucket infer_smpl{};
  AtomicLatencyBucket state2action{};
  AtomicLatencyBucket state2action_g1{};
...
void RecordPolicyTimingSample(InferenceTimingStats& stats,
  ...
  const auto previous_start_ns =
      stats.last_infer_start_ns.exchange(infer_start_system_ns,
                                         std::memory_order_acq_rel);
  if (previous_start_ns > 0 && infer_start_system_ns > previous_start_ns) {
    RecordLatencyNs(stats.policy_period,
                    infer_start_system_ns - previous_start_ns);
  }
...
void RecordProbeComparisonTimingSample(InferenceTimingStats& stats,
  ...
  RecordLatencyNs(stats.infer_g1, g1_infer_ns);
  RecordLatencyNs(stats.infer_smpl, smpl_infer_ns);
  const auto timing = CaptureStateTimingSample(state, infer_start_system_ns);
  RecordStateTimingSample(stats, timing);
  if (timing.valid) {
    RecordLatencyNs(stats.state2action_g1,
                    timing.transport_ns + timing.ready_ns + g1_infer_ns);
...
void AppendInferenceTimingLog(InferenceTimingStats& stats,
  ...
  const auto policy_period = ConsumeLatencyBucket(stats.policy_period);
  ...
  const auto infer_g1 = ConsumeLatencyBucket(stats.infer_g1);
  ...
  const auto state2action_g1 = ConsumeLatencyBucket(stats.state2action_g1);
  const auto state2action_smpl = ConsumeLatencyBucket(stats.state2action_smpl);
  if (policy_period.count == 0 &&
      transport.count == 0 && ready.count == 0 && sync_wait.count == 0 &&
      policy_wait.count == 0 && infer.count == 0 && infer_g1.count == 0 &&
      infer_smpl.count == 0 &&
      state2action.count == 0 && state2action_g1.count == 0 &&
  ...
  if (log_mode == LatencyLogMode::kCompact) {
    const auto& primary_total =
        (state2action_g1.count > 0) ? state2action_g1 : state2action;
    const auto& primary_infer =
        (infer_g1.count > 0) ? infer_g1 : infer;
    std::cout << " | lat_ms avg/max";
    AppendAvgMaxLatencyBucket("infer_dt", policy_period);
    AppendAvgMaxLatencyBucket(
        (state2action_g1.count > 0) ? "g1" : "total", primary_total);
  ...
  AppendCompactLatencyBucket("infer_dt", policy_period);
  ...
  AppendCompactLatencyBucket("infer_g1", infer_g1);
  AppendCompactLatencyBucket("infer_smpl", infer_smpl);
  AppendCompactLatencyBucket("total", state2action);
  AppendCompactLatencyBucket("total_g1", state2action_g1);
...
int main(int argc, char** argv) {
  ...
  const std::string policy_control_profile = NormalizeConfigToken(
      OptionalKey<std::string>(cfg["policy_control"]["profile"],
                               std::string{"a3"}));
  const std::array<double, 29>* policy_kps = &a3_kps;
  const std::array<double, 29>* policy_kds = &a3_kds;
  const std::array<double, 29>* policy_action_scale = &a3_action_scale;
  if (policy_control_profile == "a3_024" ||
      policy_control_profile == "a3_exp023" ||
      policy_control_profile == "exp023") {
    policy_kps = &a3_024_kps;
    policy_kds = &a3_024_kds;
    policy_action_scale = &a3_024_action_scale;
  } else if (policy_control_profile != "a3" &&
             policy_control_profile != "base") {
    std::cerr << "unsupported policy_control.profile='"
              << policy_control_profile
              << "'; expected one of: a3, a3_024\n";
    return 65;
  }
  std::cout << "✓ policy control profile selected: "
            << policy_control_profile
            << " ankle_kp=" << (*policy_kps)[21]
            << " ankle_kd=" << (*policy_kds)[21]
            << " ankle_pitch_action_scale=" << (*policy_action_scale)[21]
            << " ankle_roll_action_scale=" << (*policy_action_scale)[22]
            << "\n";
  ...
  const bool probe_manual = HasFlag(argc, argv, "--probe-manual");
  // The public 024 package ships G1 and A3-fast policy graphs but no SMPL
  // graph.  Keep direct `--probe` usable without an extra flag; callers with
  // an SMPL-capable runtime can still opt in explicitly.
  const std::string default_probe_source = "g1";
  ...
  const bool auto_start =
      HasFlag(argc, argv, "--auto-start") || (probe_mode && !probe_manual);
  ...
  if (probe_manual && !probe_mode) {
    std::cerr << "--probe-manual is only valid with --probe\n";
    return 64;
  }
  if (probe_manual && HasFlag(argc, argv, "--auto-start")) {
    std::cerr << "--probe-manual cannot be combined with --auto-start\n";
    return 64;
  }
  if (probe_source != "g1" && probe_source != "smpl" &&
      probe_source != "both") {
    std::cerr << "--probe-source must be one of: g1, smpl, both\n";
    return 64;
  }
  if (!probe_mode && probe_source != "g1") {
  ...
  if (probe_mode) {
    ...
    if (probe_manual) {
      std::cout << "✓ manual probe enabled: keyboard state machine active; "
                   "command publishers remain disabled\n";
    }
    if (probe_source_smpl) {
      std::cout << "✓ probe source: smpl monolithic policy "
                   "(synthetic zero SMPL tokenizer)\n";
    } else if (probe_source_both) {
      std::cout << "✓ probe source: g1 + smpl monolithic comparison "
                   "(synthetic zero SMPL tokenizer)\n";
    }
  }
  ...
  const int g1_encoder_index = OptionalKey<int>(
      cfg["onnx"]["encoder_indices"]["g1"], 0);
  ...
  if (encoder_decoder_mode) {
    ...
    policy = load_monolithic_policy("g1", onnx_path,
  ...
  std::shared_ptr<a3_deploy::A3MotionRpcController> motion_rpc_controller;
  if (auto* aimrt_backend =
          dynamic_cast<robot_io::A3AimrtBackend*>(backend_ptr.get())) {
    std::vector<a3_deploy::A3RpcMotionEntry> rpc_motions;
    rpc_motions.reserve(motion_library.Size());
    for (std::size_t i = 0; i < motion_library.Size(); ++i) {
      const auto& clip = motion_library.Clip(i);
      rpc_motions.push_back({
          .id = static_cast<int>(i),
          .name = clip.name,
          .path = clip.path,
          .total_ticks = clip.reference.NumTicks(),
      });
    }
    motion_rpc_controller =
        std::make_shared<a3_deploy::A3MotionRpcController>(
            manual_control, std::move(rpc_motions));
    aimrt_backend->SetMotionRpcController(motion_rpc_controller);
    std::cout << "✓ motion RPC catalog ready: " << motion_library.Size()
              << " loaded paths\n";
  }

  // --- Evaluation recorder -------------------------------------------------
  // Only enabled=true creates the output directory and writer thread.
  const YAML::Node eval_cfg = cfg["evaluation_recording"];
  a3_deploy::A3EvalRecorderOptions eval_options;
  eval_options.enabled = OptionalKey<bool>(eval_cfg["enabled"], false);
  if (eval_options.enabled) {
    eval_options.complete_motion_only =
        OptionalKey<bool>(eval_cfg["complete_motion_only"], false);
    const std::string output_root_raw = OptionalKey<std::string>(
        eval_cfg["output_root"], std::string{"eval_records"});
    eval_options.output_root = ResolveRuntimePathString(
        output_root_raw, runtime_cfg_lookup_path);
    eval_options.session_id =
        OptionalKey<std::string>(eval_cfg["session_id"], std::string{});
    eval_options.backend_name = backend.Name();
    // A3 sync 使用 system_clock；G1 SDK backend 使用 steady/monotonic clock。
    eval_options.state_clock_domain =
        backend.Name() == "a3" ? "system" : "monotonic";
    eval_options.runtime_config_path = runtime_cfg_lookup_path.string();
    eval_options.policy_model_path = encoder_decoder_mode
                                         ? encoder_model_path + " + " +
                                               decoder_model_path
                                         : onnx_path;
    eval_options.policy_hz = configured_policy_hz;
    eval_options.state_hz = OptionalKey<double>(
        cfg["backend"]["sync_hz"], configured_policy_hz * 2.0);
    eval_options.frame_queue_capacity = OptionalKey<std::size_t>(
        eval_cfg["frame_queue_capacity"], 8192ULL);
    eval_options.flush_frames =
        OptionalKey<std::size_t>(eval_cfg["flush_frames"], 512ULL);

    const YAML::Node telemetry_cfg = eval_cfg["realtime_visualization"];
    eval_options.telemetry.enabled =
        OptionalKey<bool>(telemetry_cfg["enabled"], false);
    eval_options.telemetry.publish_hz =
        OptionalKey<double>(telemetry_cfg["publish_hz"], 10.0);
    eval_options.telemetry.queue_capacity = OptionalKey<std::size_t>(
        telemetry_cfg["queue_capacity"], 256ULL);
    eval_options.telemetry.node_name = OptionalKey<std::string>(
        telemetry_cfg["node_name"], std::string{"a3_eval_telemetry"});
    eval_options.telemetry.topic_prefix = OptionalKey<std::string>(
        telemetry_cfg["topic_prefix"], std::string{"/a3/eval"});

    // 一个完整 motion 对应一个 MCAP，复用 recorder writer thread；
    // environment 显式区分 MuJoCo(sim) 与真机(real)，并进入文件名/metadata。
    const YAML::Node episode_mcap_cfg = eval_cfg["episode_mcap"];
    eval_options.episode_mcap.enabled =
        OptionalKey<bool>(episode_mcap_cfg["enabled"], false);
    eval_options.episode_mcap.environment = OptionalKey<std::string>(
        episode_mcap_cfg["environment"], std::string{"sim"});
    eval_options.episode_mcap.model_tag = OptionalKey<std::string>(
        episode_mcap_cfg["model_tag"], std::string{});
    eval_options.episode_mcap.topic_prefix = OptionalKey<std::string>(
        episode_mcap_cfg["topic_prefix"],
        eval_options.telemetry.topic_prefix);
    eval_options.episode_mcap.raw_directory = OptionalKey<std::string>(
        episode_mcap_cfg["directory"], std::string{"data"});
    const YAML::Node readable_mcap_cfg = episode_mcap_cfg["readable_copy"];
    eval_options.episode_mcap.readable_copy_enabled = OptionalKey<bool>(
        readable_mcap_cfg["enabled"], false);
    eval_options.episode_mcap.readable_directory = OptionalKey<std::string>(
        readable_mcap_cfg["directory"], std::string{"data_read"});

    for (std::size_t i = 0; i < motion_library.Size(); ++i) {
      const auto& clip = motion_library.Clip(i);
      eval_options.motions.push_back({
          static_cast<std::int32_t>(i), clip.name, clip.path.string(),
          static_cast<std::uint64_t>(clip.reference.NumTicks())});
    }
    const auto remote_base = static_cast<std::int32_t>(motion_library.Size());
    for (std::size_t i = 0; i < remote_motion_library.Size(); ++i) {
      const auto& clip = remote_motion_library.Clip(i);
      eval_options.motions.push_back({
          remote_base + static_cast<std::int32_t>(i), clip.name,
          clip.path.string(),
          static_cast<std::uint64_t>(clip.reference.NumTicks())});
    }
  }
  ...
  auto eval_recorder =
      std::make_unique<a3_deploy::A3EvalRecorder>(std::move(eval_options));
  if (eval_recorder->Enabled()) {
    std::string eval_error;
    if (!eval_recorder->Start(&eval_error)) {
      std::cerr << "evaluation recorder 启动失败: " << eval_error << "\n";
      return 1;
    }
    std::cout << "✓ evaluation recorder: "
              << eval_recorder->SessionDirectory() << "\n";
  }
  a3_deploy::EvalFrameV1 eval_pending_frame;
  bool eval_pending_valid = false;
  std::uint64_t next_eval_frame_id = 0;
  ManualMotionShortcuts motion_shortcuts;
  {
  ...
  auto policy_fn = [&](std::uint64_t                 tick_idx,
    ...
    if (eval_pending_valid) {
      eval_pending_frame.infer_start_monotonic_ns = NowMonotonicNsNoThrow();
    }
    bool infer_ok = false;
    ...
    if (encoder_decoder_mode) {
      ...
      if (compare_probe_sources) {
        obs_builder.BuildObsDict(tok, obs);
        float* g1_in_buf = policy->MutableInputData();
        for (std::size_t i = 0; i < obs.size(); ++i) g1_in_buf[i] = obs[i];
        const auto g1_start = std::chrono::steady_clock::now();
        const bool g1_ok = policy->Infer();
        const auto g1_end = std::chrono::steady_clock::now();
    ...
    if (eval_pending_valid) {
      eval_pending_frame.infer_end_monotonic_ns = NowMonotonicNsNoThrow();
      eval_pending_frame.valid_mask |= a3_deploy::kInferTimingValid;
      if (infer_ok) eval_pending_frame.quality_flags |= a3_deploy::kInferOk;
    }
    if (comparison_timing_valid) {
      RecordProbeComparisonTimingSample(infer_timing, state,
                                        infer_start_system_ns,
                                        comparison_g1_infer_ns,
    ...
    a3_deploy::DecodeAction(raw_action, *policy_action_scale, q_des_29_out);
    raw_action_valid = true;
    if (eval_pending_valid) {
      eval_pending_frame.raw_action = raw_action;
      eval_pending_frame.valid_mask |= a3_deploy::kRawActionValid;
    }
  ...
  auto teleop_source_from_value = [](int value) noexcept {
    switch (value) {
      ...
      default: return a3_deploy::A3EncoderSource::kG1;
    }
  };
  ...
  auto teleop_source_value = [](a3_deploy::A3EncoderSource source) noexcept {
    switch (source) {
      ...
      case a3_deploy::A3EncoderSource::kG1: return 0;
    }
    return 0;
  };
  ...
  auto teleop_source_available =
    ...
    switch (source) {
      case a3_deploy::A3EncoderSource::kG1:
        return true;
  ...
  // Evaluation Demo 1 的 motion id 在本 session 内统一编号：普通 motion
  // 使用 [0, normal_size)，remote motion 紧随其后。
  auto eval_active_motion_id = [&]() noexcept -> std::int32_t {
    if (active_motion_is_remote) {
      return static_cast<std::int32_t>(motion_library.Size() +
                                       active_remote_motion_index);
    }
    return static_cast<std::int32_t>(active_motion_index);
  };
  auto eval_selected_motion_id = [&]() noexcept -> std::int32_t {
    const bool remote_selected =
        manual_control.remote_motion_active.load(std::memory_order_acquire) &&
        !remote_motion_library.Empty();
    if (remote_selected) {
      const int index = manual_control.selected_remote_motion_index.load(
          std::memory_order_acquire);
      return index < 0
                 ? a3_deploy::kA3EvalNoMotion
                 : static_cast<std::int32_t>(motion_library.Size()) + index;
    }
    return manual_control.selected_motion_index.load(std::memory_order_acquire);
  };
  auto prepare_eval_frame = [&](std::uint64_t tick_idx,
                                DeployMode mode) noexcept {
    if (!eval_recorder->Enabled()) return;
    eval_pending_frame = a3_deploy::EvalFrameV1{};
    eval_pending_frame.policy_tick = tick_idx;
    eval_pending_frame.deploy_mode = mode;
    eval_pending_frame.active_motion_id = eval_active_motion_id();
    eval_pending_frame.selected_motion_id = eval_selected_motion_id();
    eval_pending_valid = true;
  };
  auto capture_eval_reference = [&](std::uint64_t reference_tick) noexcept {
    if (!eval_pending_valid || policy_reference == nullptr) return;
    a3_deploy::A3EvalReferenceSample sample;
    const double yaw_offset =
        policy_yaw_offset_valid ? policy_yaw_offset_rad : 0.0;
    if (!policy_reference->SampleEvalFrame(
            reference_tick, yaw_offset, sample)) {
      return;
    }
    eval_pending_frame.reference_tick = reference_tick;
    eval_pending_frame.q_ref = sample.q_mujoco;
    eval_pending_frame.dq_ref = sample.dq_mujoco;
    eval_pending_frame.reference_pelvis_position_m =
        sample.pelvis_position_m;
    eval_pending_frame.reference_pelvis_quat_wxyz =
        sample.pelvis_quat_wxyz;
    eval_pending_frame.valid_mask |= a3_deploy::kReferenceValid;
  };
  ...
  auto command_fn = [&](std::uint64_t               tick_idx,
    ...
    if (auto_start) {
      prepare_eval_frame(
          tick_idx, tick_idx < configured_warmup_ticks
                        ? DeployMode::kPdStand
                        : DeployMode::kMotion);
      if (tick_idx < configured_warmup_ticks) {
        if (eval_pending_valid) {
          eval_pending_frame.command_source =
              a3_deploy::CommandSource::kPdStand;
          eval_pending_frame.gain_profile_id = 1;
        }
        policy_yaw_offset_valid = false;
      ...
      if (eval_pending_valid && tick_idx >= configured_warmup_ticks) {
        eval_pending_frame.motion_phase = a3_deploy::MotionPhase::kPlaying;
        eval_pending_frame.command_source = a3_deploy::CommandSource::kPolicy;
        eval_pending_frame.gain_profile_id = 2;
        capture_eval_reference(tick_idx - configured_warmup_ticks);
      }
      ...
      if (tick_idx < configured_warmup_ticks) {
        ...
        a3_deploy::ExpandToBackend(command_q_des, *policy_kps, *policy_kds,
                                   command_out);
      }
      return true;
    }
    ...
    const DeployMode mode = LoadDeployMode(manual_control);
    prepare_eval_frame(tick_idx, mode);
    if (epoch != observed_epoch || mode != observed_mode) {
      ...
      if (mode == DeployMode::kMotion) {
        ...
        if (!teleop_source_available(active_teleop_source)) {
          ...
          active_teleop_source = a3_deploy::A3EncoderSource::kG1;
          manual_control.teleop_source.store(0, std::memory_order_release);
          std::fprintf(stderr,
                       "[teleop] %s source disabled on entry; using g1\n",
    ...
    if (mode == DeployMode::kIdle) {
      if (eval_pending_valid) {
        eval_pending_frame.command_source = a3_deploy::CommandSource::kNone;
      }
      return false;
    }
    ...
    if (mode == DeployMode::kPassive) {
      if (eval_pending_valid) {
        eval_pending_frame.command_source = a3_deploy::CommandSource::kPassive;
        eval_pending_frame.gain_profile_id = 0;
      }
    ...
    if (mode == DeployMode::kPdStand) {
      if (eval_pending_valid) {
        eval_pending_frame.command_source = a3_deploy::CommandSource::kPdStand;
        eval_pending_frame.gain_profile_id = 1;
      }
    ...
    if (mode == DeployMode::kTeleop) {
      if (eval_pending_valid) {
        eval_pending_frame.command_source = a3_deploy::CommandSource::kTeleop;
        eval_pending_frame.gain_profile_id = 2;
      }
      ...
      if (!active_source_is_smpl) {
        ...
        if (!smpl_tokenizer_ok) {
          ...
          teleop_using_fallback_reference = build_teleop_stand_fallback(
              ...
              smpl_zmq_enabled, a3_deploy::A3EncoderSource::kG1,
              tokenizer_slice);
        }
      }
      ...
      if (policy_source_ok && !teleop_using_fallback_reference &&
          !active_source_is_smpl) {
        a3_deploy::A3TeleopFrame delayed_frame{};
        if (active_ta_reference.SampleDelayedFrame(state.timestamp_ns,
                                                   delayed_frame) &&
            delayed_frame.has_head_command) {
          a3_deploy::ApplyA3HeadCommand(delayed_frame.head_q_rad, command_out);
        }
      }
      return true;
    }
    ...
    if (eval_pending_valid) {
      ...
      if (!use_idle_stand) {
        eval_pending_frame.motion_phase = a3_deploy::MotionPhase::kPlaying;
      } else if (motion_ticks > 0 &&
                 policy_tick + 1 >= motion_ticks &&
                 eval_pending_frame.active_motion_id ==
                     eval_pending_frame.selected_motion_id) {
        eval_pending_frame.motion_phase =
            a3_deploy::MotionPhase::kTerminalHold;
      } else if (eval_pending_frame.active_motion_id !=
                     eval_pending_frame.selected_motion_id ||
                 policy_tick == 0) {
        eval_pending_frame.motion_phase = a3_deploy::MotionPhase::kReady;
      } else {
        eval_pending_frame.motion_phase = a3_deploy::MotionPhase::kPaused;
      }
    }
    if (use_idle_stand) {
    ...
    capture_eval_reference(policy_tick);
  ...
  if (eval_recorder->Enabled()) {
    dopt.trace_fn = [&](const robot_io::RobotState& state,
                        const robot_io::RobotCommand* command,
                        const a3_deploy::A3PolicyDriverTrace& trace) {
      // command_fn 与 trace_fn 位于同一 driver thread，pending frame 不需要锁。
      a3_deploy::EvalFrameV1 frame;
      if (eval_pending_valid &&
          eval_pending_frame.policy_tick == trace.policy_tick) {
        frame = eval_pending_frame;
      } else {
        frame.policy_tick = trace.policy_tick;
        frame.deploy_mode = auto_start
                                ? (trace.policy_tick < configured_warmup_ticks
                                       ? DeployMode::kPdStand
                                       : DeployMode::kMotion)
                                : LoadDeployMode(manual_control);
        frame.active_motion_id = eval_active_motion_id();
        frame.selected_motion_id = eval_selected_motion_id();
      }
      eval_pending_valid = false;

      frame.frame_id = next_eval_frame_id++;
      frame.control_start_monotonic_ns = trace.control_start_monotonic_ns;
      frame.control_end_monotonic_ns = trace.control_end_monotonic_ns;
      frame.state_timestamp_ns = state.timestamp_ns;
      frame.state_data_ready_ns = state.state_data_ready_ns;
      frame.state_sync_ready_ns = state.state_sync_ready_ns;
      frame.state_tick = state.tick >= 0
                             ? static_cast<std::uint64_t>(state.tick)
                             : a3_deploy::kA3EvalNoTick;

      if (state.q.size() == robot_io::kA3Dof &&
          state.dq.size() == robot_io::kA3Dof &&
          state.tau_est.size() == robot_io::kA3Dof) {
        for (int i = 0; i < robot_io::kA3Dof; ++i) {
          frame.q[i] = static_cast<float>(state.q[i]);
          frame.dq[i] = static_cast<float>(state.dq[i]);
          frame.tau_est[i] = static_cast<float>(state.tau_est[i]);
        }
        frame.valid_mask |= a3_deploy::kRobotStateValid;
      }
      for (int i = 0; i < 4; ++i) {
        frame.pelvis_quat_wxyz[i] =
            static_cast<float>(state.imu_quat_wxyz[i]);
      }
      for (int i = 0; i < 3; ++i) {
        frame.pelvis_gyro[i] = static_cast<float>(state.imu_gyro[i]);
      }
      frame.valid_mask |= a3_deploy::kPelvisImuValid;

      // Torso IMU 是 backend 的 secondary IMU；只有收到有效 secondary IMU
      // 后才写入并置 valid bit，避免把默认零值误当成真实测量。
      if (state.has_secondary_imu) {
        for (int i = 0; i < 4; ++i) {
          frame.torso_quat_wxyz[i] =
              static_cast<float>(state.sec_imu_quat_wxyz[i]);
        }
        for (int i = 0; i < 3; ++i) {
          frame.torso_gyro[i] = static_cast<float>(state.sec_imu_gyro[i]);
        }
        frame.valid_mask |= a3_deploy::kTorsoImuValid;
      }

      if (state.sync_complete) frame.quality_flags |= a3_deploy::kSyncComplete;
      if (state.sync_aligned) frame.quality_flags |= a3_deploy::kSyncAligned;
      if (trace.watchdog_safe_halt) {
        frame.quality_flags |= a3_deploy::kWatchdogSafeHalt;
        frame.command_source = a3_deploy::CommandSource::kSafeHalt;
        frame.gain_profile_id = 3;
      }

      if (trace.send_attempted) {
        frame.send_start_monotonic_ns = trace.send_start_monotonic_ns;
        frame.send_end_monotonic_ns = trace.send_end_monotonic_ns;
        frame.valid_mask |= a3_deploy::kSendTimingValid;
      }
      if (trace.send_ok) frame.quality_flags |= a3_deploy::kCommandSent;

      if (command != nullptr && trace.command_generated) {
        ...
        if (command_shape_valid) {
          // 这里只做固定长度、无分配的数据快照。理论 PD 力矩等派生量由
          // 离线工具使用这些原始输入计算，不进入 policy/control 路径。
          for (int i = 0; i < robot_io::kA3Dof; ++i) {
            frame.command_q_des[i] = static_cast<float>(command->q_des[i]);
            frame.command_dq_des[i] = static_cast<float>(command->dq_des[i]);
            frame.command_tau_ff[i] = static_cast<float>(command->tau_ff[i]);
            frame.command_kp[i] = static_cast<float>(command->kp[i]);
            frame.command_kd[i] = static_cast<float>(command->kd[i]);
          }
          frame.valid_mask |= a3_deploy::kRobotCommandValid;
        }
      }

      // policy/driver thread 的最后一步只有固定帧快照入队；episode 推导、
      // 文件写入、降采样及 ROS2 publish 均在后级线程完成。
      eval_recorder->TryPushFrame(frame);
    };
  }
  ...
  const std::string sync_mode = NormalizeConfigToken(
      OptionalKey<std::string>(cfg["backend"]["sync_mode"], "latest_cache"));
  const bool latest_frame_mode = sync_mode == "latest_frame";
  const bool latest_cache_mode = sync_mode == "latest_cache";
  const bool latest_direct_mode = latest_frame_mode || latest_cache_mode;
  ...
  auto configure_policy_phase_alignment = [&]() -> bool {
    if (!phase_align_to_sync) {
      if (latest_cache_mode) {
        std::cout << "[policy] latest-cache mode: policy_hz=" << dopt.policy_hz
                  << " trigger=timer raw_topics=cache_pull\n";
      } else if (latest_frame_mode) {
    ...
    if (sync_mode != "header_interp" && sync_mode != "min_skew_pair" &&
        sync_mode != "latest_cache" && sync_mode != "latest_frame") {
      std::cerr << "backend.sync_mode must be one of "
                   "{header_interp,min_skew_pair,latest_cache,latest_frame}; got: "
                << sync_mode << "\n";
      return false;
    }
    ...
    if (latest_cache_mode) {
      // Cache pulls occur inside A3PolicyDriver::RunOnce on its fixed timer;
      // there is no backend release phase to align against.
      dopt.trigger_on_state = false;
      std::cout << "[policy] latest-cache mode ignores phase alignment: "
                   "trigger=timer raw_topics=cache_pull\n";
      return true;
    }
    if (latest_frame_mode) {
  ...
  if (eval_recorder->Enabled()) {
    eval_recorder->Stop();
    const auto eval_stats = eval_recorder->Statistics();
    std::cout << "evaluation recorder stopped: "
              << eval_recorder->SessionDirectory()
              << " frames=" << eval_stats.frames_written
              << " dropped=" << eval_stats.frames_dropped
              << " events=" << eval_stats.events_written
              << " telemetry=" << eval_stats.telemetry_published
              << " telemetry_dropped=" << eval_stats.telemetry_dropped
              << "\n";
  }
  if (dump_fp) {
    std::fclose(dump_fp);

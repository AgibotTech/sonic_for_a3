...
Usage:
  ...
  ./run_a3_body_drive_debug_convert.sh bags/a3_body_drive_debug/<timestamp>/raw --no-raw --mesh-mode data-uri
  ./run_a3_body_drive_debug_convert.sh bags/a3_body_drive_debug/<timestamp>/raw --no-raw --mesh-url-base http://127.0.0.1:8765/assets/a3

Converts AimRT raw body-drive MCAP files into a Foxglove-friendly MCAP plus
layout/settings files.

By default, mesh files are embedded as MCAP attachments and referenced from
the URDF with package:// URLs, so Foxglove Desktop can open the result without

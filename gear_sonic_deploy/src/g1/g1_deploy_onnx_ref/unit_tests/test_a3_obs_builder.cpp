...
using a3_deploy::kA3RawActionClip;
...
// gear_sonic/envs/manager_env/mdp/observations.py — see the header
...
TEST(A3ObsBuilder, SmplObsDictPacksSmplTokenizerAndProprio) {
  ...
  std::array<float, kA3ObsDictTotalFloats> g1_out{};
  b.BuildObsDict(nullptr, g1_out);
  for (std::size_t i = 0; i < kA3ProprioTotalFloats; ++i) {
    EXPECT_FLOAT_EQ(out[kA3SmplTokenizerTotalFloats + i],
                    g1_out[kA3TokenizerTotalFloats + i])
        << "proprio i=" << i;
  }
}
...
TEST(A3ObsBuilder, RememberClipsActionBeforeCaching) {
  A3ObsBuilder b;
  std::array<float, 29> raw_isaaclab{};
  raw_isaaclab.fill(0.25f);
  raw_isaaclab[0] = kA3RawActionClip + 0.5f;
  raw_isaaclab[5] = -kA3RawActionClip - 0.5f;

  b.RememberAction(raw_isaaclab);
  const auto last_mujoco = b.LastAction();
  for (int i = 0; i < 29; ++i) {
    const int src = a3_isaaclab_to_mujoco[i];
    const float expected = std::clamp(
        raw_isaaclab[src], -kA3RawActionClip, kA3RawActionClip);
    EXPECT_DOUBLE_EQ(last_mujoco[i], static_cast<double>(expected));
  }

  ProprioFrame f{};
  f.act_mujoco = last_mujoco;
  PushFrame(b, f);
  const auto tok = MakeTokenizerSlice(0.0f);
  std::array<float, kA3ObsDictTotalFloats> out{};
  b.BuildObsDict(tok.data(), out);
  const float* proprio = out.data() + kA3TokenizerTotalFloats;
  constexpr std::size_t newest = kA3ProprioHistoryLength - 1;
  for (int i = 0; i < 29; ++i) {
    const float expected = std::clamp(
        raw_isaaclab[i], -kA3RawActionClip, kA3RawActionClip);
    EXPECT_FLOAT_EQ(proprio[kTermOfsActions + newest * 29 + i], expected);
  }
}

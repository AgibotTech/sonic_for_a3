...
// than G1's flat layout (legs+waist+arms+neck). The old "first 29 names
// match G1" assertion is intentionally gone — A3 and G1 layouts differ.
...
TEST(A3LayoutExtra, NeckMappingIsIdentity) {
  EXPECT_EQ(robot_io::kG1ToA3NeckIdx[0], 0);
  EXPECT_EQ(robot_io::kG1ToA3NeckIdx[1], 1);
  EXPECT_EQ(robot_io::kA3NeckToG1Idx[0], 0);
  EXPECT_EQ(robot_io::kA3NeckToG1Idx[1], 1);
}

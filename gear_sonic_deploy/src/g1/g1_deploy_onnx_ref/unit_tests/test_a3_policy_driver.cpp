...
class CapturingBackend final : public robot_io::RobotIOBackend {
 public:
  ...
  bool UsesLatestStatePull() const noexcept override {
    return pull_enabled_.load(std::memory_order_acquire);
  }

  bool TryReadLatestState(RobotState& state) override {
    pull_reads_.fetch_add(1, std::memory_order_relaxed);
    std::lock_guard<std::mutex> lk(mu_);
    if (!has_pulled_state_) return false;
    state = pulled_state_;
    return true;
  }
  ...
  void SetPulledState(const RobotState& state) {
    std::lock_guard<std::mutex> lk(mu_);
    pulled_state_ = state;
    has_pulled_state_ = true;
    pull_enabled_.store(true, std::memory_order_release);
  }

  std::uint64_t PullReadCount() const {
    return pull_reads_.load(std::memory_order_relaxed);
  }
 ...
 private:
  ...
  RobotState pulled_state_{};
  bool has_pulled_state_{false};
  std::atomic<bool> pull_enabled_{false};
  std::atomic<std::uint64_t> pull_reads_{0};
};
...
TEST(A3PolicyDriver, LatestStatePullRunsWithoutBackendStateCallbacks) {
  CapturingBackend backend;
  backend.SetPulledState(MakeState(true, true));

  A3PolicyDriverOptions opt;
  opt.policy_hz = 100.0;
  opt.sched.priority = 0;
  std::atomic<std::uint64_t> policy_calls{0};
  A3PolicyDriver driver(
      backend,
      [&](std::uint64_t, const RobotState&, std::array<double, 29>& out) {
  ...
  ASSERT_TRUE(WaitUntil(
      [&] { return policy_calls.load(std::memory_order_relaxed) >= 2; },
      std::chrono::milliseconds(200)));
  driver.StopDriver();

  EXPECT_GE(backend.PullReadCount(), 2u);
...
TEST(ExpandToBackend, ScattersBodyToNonNeckSlotsAndControlsHead) {
  ...
  // Head/neck slots [3..4] — fixed zero-position PD command by default.
...
TEST(ExpandToBackend, ApplyA3HeadCommandOverlaysTeleopHeadSlots) {
  auto q   = MakeLinearArray(0.1, 0.01);
  auto kps = MakeLinearArray(10.0, 1.0);
  auto kds = MakeLinearArray(1.0, 0.1);
  RobotCommand cmd;
  ...
  const int first_body_sdk = robot_io::kA3PolicyToSdkIdx[0];
  const double body_q_before = cmd.q_des[first_body_sdk];
  const double body_kp_before = cmd.kp[first_body_sdk];

  a3_deploy::ApplyA3HeadCommand({0.31, -0.22}, cmd);

  EXPECT_DOUBLE_EQ(cmd.q_des[robot_io::kA3NeckStart + 0], 0.31);
  EXPECT_DOUBLE_EQ(cmd.q_des[robot_io::kA3NeckStart + 1], -0.22);
  for (int i = 0; i < robot_io::kA3NeckCount; ++i) {
    const int sdk = robot_io::kA3NeckStart + i;
    EXPECT_DOUBLE_EQ(cmd.dq_des[sdk], 0.0);
    EXPECT_DOUBLE_EQ(cmd.tau_ff[sdk], 0.0);
    EXPECT_DOUBLE_EQ(cmd.kp[sdk], a3_deploy::kA3HeadKp);
    EXPECT_DOUBLE_EQ(cmd.kd[sdk], a3_deploy::kA3HeadKd);
  }
  EXPECT_DOUBLE_EQ(cmd.q_des[first_body_sdk], body_q_before);
  EXPECT_DOUBLE_EQ(cmd.kp[first_body_sdk], body_kp_before);
}
...
TEST(A3PolicyDriver, TraceHookSeesStateFinalCommandAndTiming) {
  CapturingBackend backend;
  ASSERT_TRUE(backend.Init(""));

  auto policy = [](std::uint64_t, const RobotState&,
                   std::array<double, 29>& out) { out.fill(0.15); };
  std::atomic<int> trace_calls{0};
  std::atomic<bool> saw_command{false};
  std::atomic<bool> timing_order_ok{false};

  A3PolicyDriverOptions opt;
  opt.policy_hz = 100.0;
  opt.watchdog.max_frame_age_ns = 500'000'000;
  opt.trace_fn = [&](const RobotState& state, const RobotCommand* command,
                     const a3_deploy::A3PolicyDriverTrace& trace) {
    trace_calls.fetch_add(1, std::memory_order_relaxed);
    if (state.tick == 42 && command != nullptr &&
        command->q_des.size() == robot_io::kA3Dof &&
        trace.command_generated && trace.send_attempted && trace.send_ok) {
      saw_command.store(true, std::memory_order_relaxed);
    }
    if (trace.control_start_monotonic_ns <= trace.send_start_monotonic_ns &&
        trace.send_start_monotonic_ns <= trace.send_end_monotonic_ns &&
        trace.send_end_monotonic_ns <= trace.control_end_monotonic_ns) {
      timing_order_ok.store(true, std::memory_order_relaxed);
    }
  };

  A3PolicyDriver driver(backend, policy, opt);
  ASSERT_TRUE(driver.StartDriver());
  auto state = MakeState(/*complete=*/true, /*aligned=*/true);
  state.tick = 42;
  backend.Emit(state);
  ASSERT_TRUE(WaitUntil(
      [&] { return trace_calls.load(std::memory_order_relaxed) > 0; },
      std::chrono::milliseconds(100)));
  driver.StopDriver();

  EXPECT_TRUE(saw_command.load(std::memory_order_relaxed));
  EXPECT_TRUE(timing_order_ok.load(std::memory_order_relaxed));
}
...
void FeedSamples(A3AimrtBackend& b, std::atomic<bool>& stop) {
  while (!stop.load()) {
    const auto t = NowNs();
    // Coherent min-skew tests model a valid upstream/header timestamp rather
    // than relying on the latest-cache fallback for unstamped samples.
    auto stamp_sample = [&](auto& sample) {
      sample.stamp = {t};
      sample.recv_stamp = {t};
      sample.source_stamp_valid = true;
    };
    a3_sync::WaistSample w{}; stamp_sample(w);
    a3_sync::LegSample   l{}; stamp_sample(l);
    a3_sync::ArmSample   a{}; stamp_sample(a);
    a3_sync::NeckSample  n{}; stamp_sample(n);
    a3_sync::ImuSample   p{}; stamp_sample(p);
    a3_sync::ImuSample   to{}; stamp_sample(to);
...
TEST(A3PolicyDriver, RunsPolicyAndSendsExpanded31DofCommand) {
  ...
  ASSERT_TRUE(backend.Init(
      "sync_mode=min_skew_pair,sync_hz=200,align_delay_ms=1,max_skew_ms=10"));

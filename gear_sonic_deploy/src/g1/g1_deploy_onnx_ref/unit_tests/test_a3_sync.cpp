...
static a3_sync::SyncConfig LatestCacheTestConfig() {
  auto cfg = LatestFrameTestConfig();
  cfg.sync_mode = a3_sync::SyncMode::LatestCache;
  return cfg;
}
...
TEST(A3SyncLoop, LatestCacheOnlyBuildsStateWhenPolicyPulls) {
  a3_sync::A3SyncLoop::Options opt;
  opt.sync = LatestCacheTestConfig();
  A3SyncLoop loop(std::move(opt));
  ...
  constexpr std::int64_t kBase = 7'000'000'000;
  const auto recv = SystemNowNs();
  loop.Start();
  loop.OnWaistState(MakeWaistSample(kBase + 0, recv + 10, 1.0));
  loop.OnLegState(MakeLegSample(kBase + 1'000'000, recv + 20));
  loop.OnArmState(MakeArmSample(kBase + 2'000'000, recv + 30));
  loop.OnNeckState(MakeNeckSample(kBase + 500'000, recv + 40));
  loop.OnPelvisImu(MakeImuSample(kBase + 1'500'000, recv + 50, 1.5));
  ...
  // Raw callbacks only replace their own cache; no full RobotState is built.
  EXPECT_EQ(callbacks.load(std::memory_order_relaxed), 0);
  EXPECT_EQ(loop.Statistics().tick_total, 0u);

  robot_io::RobotState observed;
  ASSERT_TRUE(loop.TryReadLatestState(observed));
  EXPECT_EQ(callbacks.load(std::memory_order_relaxed), 0);
  EXPECT_TRUE(observed.sync_complete);
  EXPECT_TRUE(observed.sync_aligned);
  EXPECT_EQ(observed.timestamp_ns, kBase);
  EXPECT_EQ(observed.state_data_ready_ns, recv + 60);
  EXPECT_DOUBLE_EQ(observed.q(0), 1.0);
  EXPECT_DOUBLE_EQ(observed.imu_gyro(0), 1.5);

  // A newer one-channel raw message becomes visible on the next policy pull.
  // Its >3 ms offset remains a diagnostic skew, not a control gate.
  loop.OnWaistState(MakeWaistSample(kBase + 6'000'000, recv + 70, 3.0));
  ASSERT_TRUE(loop.TryReadLatestState(observed));
  EXPECT_EQ(observed.timestamp_ns, kBase + 500'000);
  EXPECT_DOUBLE_EQ(observed.q(0), 3.0);
  EXPECT_TRUE(observed.sync_aligned);
  EXPECT_EQ(observed.sync_skew_ns, 5'500'000);
  EXPECT_EQ(loop.Statistics().tick_total, 2u);
  loop.Stop();
}

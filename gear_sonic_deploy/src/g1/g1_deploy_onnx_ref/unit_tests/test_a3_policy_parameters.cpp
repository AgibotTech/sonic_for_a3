...
TEST(A3PolicyParameters, ArraySizesAre29) {
  ...
  EXPECT_EQ(a3_024_kps.size(), 29u);
  EXPECT_EQ(a3_024_kds.size(), 29u);
  EXPECT_EQ(a3_024_action_scale.size(), 29u);
...
TEST(A3PolicyParameters, Release024MatchesTrainingActuatorContract) {
  // 024 retains the base gains except for both ankle axes.
  for (int i = 0; i < 29; ++i) {
    const bool is_ankle =
        i == 21 || i == 22 || i == 27 || i == 28;
    EXPECT_DOUBLE_EQ(a3_024_kps[i], is_ankle ? 60.0 : a3_kps[i])
        << "i=" << i;
    EXPECT_DOUBLE_EQ(a3_024_kds[i], is_ankle ? 1.2 : a3_kds[i])
        << "i=" << i;
  }

  // A3_024_ACTION_SCALE = 0.25 * effort_limit_sim / stiffness.
  EXPECT_DOUBLE_EQ(a3_024_action_scale[0], 0.25 * 200.0 / 85.0);
  EXPECT_DOUBLE_EQ(a3_024_action_scale[1], 0.25 * 42.0 / 50.0);
  EXPECT_DOUBLE_EQ(a3_024_action_scale[2], 0.25 * 105.0 / 50.0);
  EXPECT_DOUBLE_EQ(a3_024_action_scale[20], 0.25 * 300.0 / 250.0);
  EXPECT_DOUBLE_EQ(a3_024_action_scale[21], 0.25 * 118.2 / 60.0);
  EXPECT_DOUBLE_EQ(a3_024_action_scale[22], 0.25 * 54.75 / 60.0);
  EXPECT_DOUBLE_EQ(a3_024_action_scale[26], 0.25 * 300.0 / 250.0);
  EXPECT_DOUBLE_EQ(a3_024_action_scale[27], 0.25 * 118.2 / 60.0);
  EXPECT_DOUBLE_EQ(a3_024_action_scale[28], 0.25 * 54.75 / 60.0);
}
...
TEST(A3PolicyParameters, Vr3PointValid) {
  ...
  // Body offsets from sonic_a3.yaml.

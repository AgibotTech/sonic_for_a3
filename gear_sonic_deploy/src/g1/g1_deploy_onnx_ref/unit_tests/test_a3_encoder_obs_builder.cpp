...
TEST(A3EncoderObsBuilder, EncoderInputPacksIndexG1AndSmplSlices) {
  const auto g1 = Sequence<a3_deploy::kA3TokenizerTotalFloats>(1000.0f);
  ...
  a3_deploy::BuildA3EncoderInput(1, g1.data(), smpl.data(), out);
...
TEST(A3EncoderObsBuilder, EncoderInputZerosInactiveSource) {
  ...
  for (std::size_t i = 1; i <= a3_deploy::kA3TokenizerTotalFloats; ++i) {
    EXPECT_FLOAT_EQ(out[i], 0.0f) << "g1 slot " << i;
  }

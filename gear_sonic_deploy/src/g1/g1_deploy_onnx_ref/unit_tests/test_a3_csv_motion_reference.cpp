...
TEST_F(A3CsvMotionReference, SamplesEvaluationReferenceInMujocoOrder) {
  WriteCsvWithYaw({0.0, 30.0, 60.0, 90.0});
  ...
  ASSERT_TRUE(ref.SampleEvalFrame(1, sample));
  for (int mj = 0; mj < robot_io::kA3PolicyDof; ++mj) {
    EXPECT_NEAR(sample.q_mujoco[mj], SyntheticMujocoRad(1, mj), 1e-5);
    EXPECT_NEAR(sample.dq_mujoco[mj], 50.0, 2e-4);
  }
  EXPECT_NEAR(sample.pelvis_position_m[0], 1.01, 1e-6);
  EXPECT_NEAR(sample.pelvis_position_m[1], 2.0, 1e-6);
  EXPECT_NEAR(sample.pelvis_position_m[2], 3.0, 1e-6);
  const auto expected_quat = QuatFromYawDeg(30.0);
  for (std::size_t i = 0; i < expected_quat.size(); ++i) {
    EXPECT_NEAR(sample.pelvis_quat_wxyz[i], expected_quat[i], 1e-6);
  }

  a3_deploy::A3EvalReferenceSample aligned_sample;
  ASSERT_TRUE(ref.SampleEvalFrame(1, 30.0 * kDegToRad, aligned_sample));
  const double c30 = std::cos(30.0 * kDegToRad);
  const double s30 = std::sin(30.0 * kDegToRad);
  EXPECT_NEAR(
      aligned_sample.pelvis_position_m[0], c30 * 1.01 - s30 * 2.0, 1e-6);
  EXPECT_NEAR(
      aligned_sample.pelvis_position_m[1], s30 * 1.01 + c30 * 2.0, 1e-6);
  EXPECT_NEAR(aligned_sample.pelvis_position_m[2], 3.0, 1e-6);
  const auto expected_aligned_quat = QuatFromYawDeg(60.0);
  for (std::size_t i = 0; i < expected_aligned_quat.size(); ++i) {
    EXPECT_NEAR(
        aligned_sample.pelvis_quat_wxyz[i], expected_aligned_quat[i], 1e-6);
  }
  EXPECT_FALSE(ref.SampleEvalFrame(3, sample));
}
...
TEST(A3CsvMotionReferenceRealData, LoadsRepoA3CsvWhenAvailable) {
  ...
  const std::string path =
      env_path ? std::string(env_path)
               : std::string("a3_data/agibot_a3/001_walk_front_slow.csv");

...
TEST(A3AimrtBackend, InitUsesLatestCacheByDefault) {
#ifndef ENABLE_A3_AIMRT_BACKEND
  A3AimrtBackend b;
  ASSERT_TRUE(b.Init(""));
  EXPECT_TRUE(b.UsesLatestStatePull());
#else
  GTEST_SKIP() << "ENABLE_A3_AIMRT_BACKEND=ON requires cfg_file_path";
#endif
}
...
TEST(A3AimrtBackend, InitAcceptsLatestCacheSyncMode) {
  A3AimrtBackend b;
#ifdef ENABLE_A3_AIMRT_BACKEND
  ASSERT_TRUE(b.Init(
      "cfg_file_path=gear_sonic_deploy/src/g1/g1_deploy_onnx_ref/config/"
      "a3_aimrt_config.yaml,"
      "sync_mode=latest_cache,sync_hz=100"));
#else
  ASSERT_TRUE(b.Init("sync_mode=latest_cache,sync_hz=100"));
#endif
  EXPECT_TRUE(b.UsesLatestStatePull());
  EXPECT_DOUBLE_EQ(b.StateRateHz(), 100.0);
}
...
TEST(A3AimrtBackend, EndToEndSampleInjectionTriggersStateCallback) {
  ...
  std::thread feeder([&] {
    while (!stop.load()) {
      const auto t = NowNs();
      // MinSkewPair intentionally accepts only a producer/header stamp.  Set
      // both timestamp domains exactly as a subscriber conversion would.
...
TEST(BackendFactory, RejectsRemovedG1Backend) {
  EXPECT_EQ(CreateBackend("g1"), nullptr);
}

...
TEST(ConvertMessage, NeckAcceptsUltraHeadPitchAlias) {
  std::array<std::string, robot_io::kA3NeckCount> names{};
  std::array<double, robot_io::kA3NeckCount> pos{}, vel{}, eff{};
  names[0] = "ultra_head_pitch_joint";
  names[1] = "head_yaw_joint";
  pos[0] = -0.25;
  pos[1] = 0.5;
  vel[0] = -1.25;
  vel[1] = 1.5;
  ...
  ASSERT_TRUE(a3_io::NeckStateSub::ConvertMessage(names, pos, vel, eff, 4,
                                                  out));
  EXPECT_DOUBLE_EQ(out.pos[0], 0.5);
  EXPECT_DOUBLE_EQ(out.pos[1], -0.25);
  EXPECT_DOUBLE_EQ(out.vel[0], 1.5);
  EXPECT_DOUBLE_EQ(out.vel[1], -1.25);
  EXPECT_DOUBLE_EQ(out.eff[0], 2.5);
  EXPECT_DOUBLE_EQ(out.eff[1], -2.25);
  EXPECT_EQ(out.stamp.value, 4);
}

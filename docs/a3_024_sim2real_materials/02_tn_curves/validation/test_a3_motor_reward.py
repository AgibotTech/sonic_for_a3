...
TRAIN_LAUNCHER = (
    Path(__file__).resolve().parents[1] / "train_a3_024_fromscratch.sh"
)
...
def test_024_training_config_adds_motor_constraints():
    exp_config = "manager/universal_token/all_modes/sonic_a3_024"
    with initialize_config_dir(version_base="1.1", config_dir=str(CONFIG_DIR)):
        cfg = compose(config_name="base", overrides=[f"+exp={exp_config}"])

    assert cfg.exp_var == "a3_024_motor_constraints_g1_a3fast_f10"
...
def test_024_launcher_uses_weight_warm_start_without_full_state_resume():
    source = TRAIN_LAUNCHER.read_text()

    assert "manager/universal_token/all_modes/sonic_a3_024" in source
